...
kaggle_df = pd.read_csv('data/dataset_with_country.csv')
...
if len(df) > 20:
    ...
    X_base = df[num_features + ['explicit']]
    
    # Converting 'country_name' to one-hot encoding
    countries_df = pd.get_dummies(df['country_name'], prefix='country')
    
    # Joining the base features with the country one-hot encoded features
    X = pd.concat([X_base, countries_df], axis=1)
    y = df['Liked']
    ...
    def recommend_songs(model, all_songs_df, user_history_df, top_n=10):
        ...
        X_candidates_base = candidates[num_features + ['explicit']]
        
        # Country one-hot encoding for candidates
        countries_candidates = pd.get_dummies(candidates['country_name'], prefix='country')
        X_candidates = pd.concat([X_candidates_base, countries_candidates], axis=1)
        X_candidates.columns = X_candidates.columns.astype(str)
        X_candidates = X_candidates.reindex(columns=X.columns, fill_value=0)
